...
const qualityMeta: Record<string, { tr: string; en: string; badge: string }> = {
  MUKEMMEL: { tr: 'Mukemmel', en: 'Excellent', badge: 'bg-sky-100 text-sky-700' },
  IYI: { tr: 'Iyi', en: 'Good', badge: 'bg-emerald-100 text-emerald-700' },
  YETERLI: { tr: 'Yeterli', en: 'Sufficient', badge: 'bg-amber-100 text-amber-700' },
  ZAYIF: { tr: 'Zayif', en: 'Poor', badge: 'bg-rose-100 text-rose-700' },
};
...
const getSwimApiErrorMessage = (error: unknown, lang: Language): string => {
  ...
  if (message.includes('500') && message.toLowerCase().includes('missing swimming_api_user')) {
    return lang === 'tr'
      ? 'Yuzme API proxy ayarli degil. .env.local dosyasina SWIMMING_API_USER ve SWIMMING_API_PASSWORD ekleyin.'
      : 'Swimming API proxy is not configured. Add SWIMMING_API_USER and SWIMMING_API_PASSWORD to .env.local.';
  }

  if (message.includes('401') || message.includes('403')) {
    return lang === 'tr'
      ? 'Yuzme API kimlik dogrulamasi basarisiz. SWIMMING_API_USER/SWIMMING_API_PASSWORD degerlerini kontrol edin.'
      : 'Swimming API authentication failed. Check SWIMMING_API_USER/SWIMMING_API_PASSWORD values.';
  }

  return lang === 'tr' ? 'Yuzme API verisine ulasilamadi.' : 'Could not reach swimming API data.';
};
...
const SwimmingQualityPanel: React.FC<Props> = ({ lang }) => {
  ...
  const handleAreaClick = async (id: string) => {
    ...
    try {
      ...
      setDetailError(lang === 'tr' ? 'Detay verisi alinamadi.' : 'Detail data could not be fetched.');
    } finally {
    ...
    <section className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm border border-slate-100">
      <div className="flex items-center gap-2 mb-5">
        ...
        <div>
          <h3 className="text-sm font-black text-slate-900 tracking-tight">
            {lang === 'tr' ? 'Yuzme Suyu Kalitesi' : 'Swimming Water Quality'}
          </h3>
          <p className="text-[11px] text-slate-500 font-medium">
            {lang === 'tr'
              ? 'Veri kaynagi: T.C. Saglik Bakanligi Yuzme Suyu Takip Sistemi'
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mb-4">
        ...
        <div className="flex items-end">
          ...
          >
            {loadingAreas ? (lang === 'tr' ? 'YUKLENIYOR...' : 'LOADING...') : (lang === 'tr' ? 'YUZME ALANI GETIR' : 'GET AREAS')}
          </button>
        </div>
      </div>

      {error && <p className="text-sm text-rose-600 font-bold mb-3">{error}</p>}

      {areas.length > 0 && (
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 mb-4">
          <div className="bg-slate-50 border border-slate-100 rounded-xl p-3">
            <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">{lang === 'tr' ? 'Gosterilen' : 'Shown'}</p>
            <p className="text-lg font-black text-slate-800">{areas.length}</p>
          </div>
          <div className="bg-sky-50 border border-sky-100 rounded-xl p-3">
            <p className="text-[8px] font-black text-sky-500 uppercase tracking-widest">{lang === 'tr' ? 'Mukemmel' : 'Excellent'}</p>
            <p className="text-lg font-black text-sky-700">{qualityCounts.excellent}</p>
          </div>
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3">
            <p className="text-[8px] font-black text-emerald-500 uppercase tracking-widest">{lang === 'tr' ? 'Iyi' : 'Good'}</p>
          ...
          <div className="bg-rose-50 border border-rose-100 rounded-xl p-3">
            <p className="text-[8px] font-black text-rose-500 uppercase tracking-widest">{lang === 'tr' ? 'Zayif' : 'Poor'}</p>
      ...
      {areas.length > 0 && (
        <div className="space-y-2 max-h-[460px] overflow-y-auto pr-1">
          {areas.map((area) => {
            ...
            const areaDetail = detailById[area.id];
            const isDetailLoading = isSelected && loadingDetail && !areaDetail;
            ...
            return (
              <div key={area.id} className="space-y-2">
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 mt-2">
                    <span className="text-[10px] text-slate-600 font-bold">
                      {lang === 'tr' ? 'Son Sonuc:' : 'Last Result:'} {area.sampleResultExplanation || '-'}
                    </span>
                    {area.hasBlueFlag && (
                  ...
                  <p className="mt-2 text-[10px] font-black uppercase tracking-widest text-cyan-700">
                    {isSelected
                      ? (lang === 'tr' ? 'Detayi Kapat' : 'Close Details')
                      : (lang === 'tr' ? 'Detaylari Ac' : 'Open Details')}
                  </p>
                </button>
                ...
                {isSelected && detailError && <p className="text-sm text-rose-600 font-bold">{detailError}</p>}

                {isSelected && areaDetail && !isDetailLoading && (
                  <div className="p-4 sm:p-5 rounded-2xl border border-cyan-100 bg-cyan-50/40 space-y-3">
                    <div className="flex items-start justify-between gap-3">
                      <div>
                        <h4 className="text-base font-black text-slate-900 tracking-tight">{areaDetail.name}</h4>
                        <p className="text-xs text-slate-600 font-semibold">
                          {areaDetail.townName}, {areaDetail.cityName}
                        </p>
                      </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      <div className="bg-white rounded-xl p-2.5 border border-slate-100">
                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{lang === 'tr' ? 'Dus' : 'Shower'}</p>
                        <p className="text-sm font-black text-slate-800">{areaDetail.numberOfShowers ?? '-'}</p>
                      </div>
                      <div className="bg-white rounded-xl p-2.5 border border-slate-100">
                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{lang === 'tr' ? 'Kabin' : 'Cabin'}</p>
                        <p className="text-sm font-black text-slate-800">{areaDetail.numberOfCabins ?? '-'}</p>
                      </div>
                      <div className="bg-white rounded-xl p-2.5 border border-slate-100">
                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{lang === 'tr' ? 'Cankurtaran' : 'Lifeguard'}</p>
                        <p className="text-sm font-black text-slate-800">{areaDetail.numberOfLifeguards ?? '-'}</p>
                      </div>
                      <div className="bg-white rounded-xl p-2.5 border border-slate-100">
                        <p className="text-[8px] font-black uppercase tracking-widest text-slate-400">{lang === 'tr' ? 'Su Tipi' : 'Water Type'}</p>
                        <p className="text-sm font-black text-slate-800 truncate">{areaDetail.waterTypeName || '-'}</p>
                      </div>
                    </div>

                    <div className="space-y-1.5 text-xs text-slate-700 font-medium">
                      <p><span className="font-black text-slate-900">{lang === 'tr' ? 'Adres:' : 'Address:'}</span> {areaDetail.swimmingAreaAdress || '-'}</p>
                      <p><span className="font-black text-slate-900">{lang === 'tr' ? 'Isletme:' : 'Operator:'}</span> {areaDetail.businessName || '-'}</p>
                      <p><span className="font-black text-slate-900">{lang === 'tr' ? 'Telefon:' : 'Phone:'}</span> {areaDetail.businessPhone || '-'}</p>
                      <p><span className="font-black text-slate-900">{lang === 'tr' ? 'En Yakin Saglik Tesisi:' : 'Nearest Health Facility:'}</span> {areaDetail.nearestHealthFacility || '-'}</p>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs font-semibold text-slate-700">
                      <p className="bg-white border border-slate-100 rounded-xl px-3 py-2">
                        <span className="font-black text-slate-900">{lang === 'tr' ? 'Sezon Baslangici:' : 'Season Start:'}</span> {formatDate(areaDetail.swimmingSeasonStartDate, lang)}
                      </p>
                      <p className="bg-white border border-slate-100 rounded-xl px-3 py-2">
                        <span className="font-black text-slate-900">{lang === 'tr' ? 'Sezon Bitisi:' : 'Season End:'}</span> {formatDate(areaDetail.swimmingSeasonEndDate, lang)}
                      </p>
                    </div>
                  </div>
                )}
      ...
      {!loadingAreas && !error && areas.length === 0 && (
        <p className="text-sm text-slate-500 font-medium">
          {totalCount === 0
            ? (lang === 'tr' ? 'Filtre secip yuzme alanlarini getir butonuna tiklayin.' : 'Select filters and click get areas.')
            : ''}
        </p>
      )}
      ...
      <div className="mt-4 text-right">
        ...
        >
          {lang === 'tr' ? 'Resmi Sistemi Ac' : 'Open Official System'}
        </a>
      </div>
    </section>
  );
